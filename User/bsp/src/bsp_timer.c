...
extern void bsp_RunPer7ms(void);
extern void bsp_RunPer15ms(void);
void SysTick_ISR(void)
{
	static uint8_t s_count = 0;
	static uint8_t s_count_2 = 0;
	uint8_t i;
	...
	{
		s_count = 0;
		bsp_RunPer7ms();	/* 每隔7ms调用一次此函数，此函数在 bsp.c */
	}
	
	if(++s_count_2 >= 15)	
	{
		s_count_2 = 0;
		bsp_RunPer15ms();	/* 每隔15ms调用一次此函数，此函数在 bsp.c */
	}
}
...
{
	SysTick_ISR();		//加上外部中断这个systick中断会不执行
}
...
void bsp_InitHardTimer(void)
{
    ...
    /*-----------------------------------------------------------------------
		...
		----------------------------------------------------------------------- */
	uiTIMxCLK = SystemCoreClock / 2;	/* APB1时钟 84M */

	usPrescaler = uiTIMxCLK / 1000000 ;	/* 分频到周期 1us 频率1M*/
...
*	功能说明: 使用TIM2-5做单次定时器使用, 定时时间到后执行回调函数。可以同时启动4个通道，互不干扰。
